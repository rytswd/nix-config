...
Configuration is about to start...
...
ssh -o PubkeyAuthentication=no \
    ...
    "root@${NIXOS_IP}" " \
		...
		mkfs.ext4 -L nixos /dev/${NIXOS_BLOCK_DEVICE}1; \
		mkswap -L swap /dev/${NIXOS_BLOCK_DEVICE}2; \
		mkfs.fat -F 32 -n boot /dev/${NIXOS_BLOCK_DEVICE}3; \
