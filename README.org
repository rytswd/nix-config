...
nix \
    run \
    github:rytswd/nix-config
#+end_src

The bootstrap defaults to the ~ryota@coder~ / ~ryota@coder-aarch64~ profile based on CPU architecture. On any other host, pass the profile explicitly:
