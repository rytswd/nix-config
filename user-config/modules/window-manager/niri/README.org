...
NOTE: Noctalia v5 (alpha) renamed the binary from ~noctalia-shell~ to ~noctalia~. Kept here untangled (=:tangle no=) until v5 is ready for daily use; swap the two blocks when migrating.
#+begin_src kdl :tangle no
...
NOTE: Noctalia v5 (alpha) replaced the ~ipc call~ interface with ~noctalia msg ...~. Kept untangled until v5 is ready -- the explicit =:noweb-ref v5-archive= keeps it out of the tangled =<<key-bindings-commands>>= expansion (otherwise it would inherit this section's =:noweb-ref key-bindings-commands=). v5 has no documented dedicated emoji / notification-history panel command yet, so those map to the nearest equivalents.
#+begin_src kdl :tangle no :noweb-ref v5-archive
  ...
  Super+Alt+Backslash { spawn-sh "noctalia msg panel-toggle launcher"; }
  Super+Shift+Backslash { spawn-sh "noctalia msg panel-toggle control-center"; }
#+end_src
