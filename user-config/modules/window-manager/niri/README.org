...
#+begin_src kdl
  Alt+Space     { spawn "sh" "-c" "rofi -show drun"; }
#+end_src
...
Use ~fuzzel~ for the clipboard history. I do use ~rofi~ for the main launcher now, but for clipboard history, I do not need the column layout, and that is easier handled with a simple setup with ~fuzzel~.
...
#+begin_src kdl
  Super+Alt+K   { spawn "sh" "-c" \
                  ...
                  ; } // Dvorak V
  Super+Alt+Backslash { spawn "sh" "-c" \
                  "bemoji -t -c -n" ; }
