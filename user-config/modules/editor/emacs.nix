...

{
  ...
  config =
    ...
    in
    lib.mkIf config.editor.emacs.enable {
      ...
      home.packages =
        let
          ...
          emacs-unstable-nixos =
            emacsPkgs.emacs-unstable-pgtk |> withNixOSOverride |> withPackages;
          # Emacs git refers to the latest master build, and thus could be flaky.
          emacs-master-nixos =
            emacsPkgs.emacs-git-pgtk |> withNixOSOverride |> withPackages |> withWrapper;
