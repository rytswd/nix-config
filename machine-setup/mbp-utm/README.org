...
NOTE: The guest NixOS configuration lives in ~nixos-config/mbp-vm~, which is shared with the Parallels-hosted variant -- the flake exposes ~#nixos-utm~ and ~#nixos-parallels~ from the same directory, differing only in the guest-tools leaf and hostname. For the Parallels flow (including the work-machine constraints: no Nix on the host, no private-input credential), see [[../mbp-parallels/README.org]].
...
As of writing in Jan 2024, there is an open issue for this (re-checked July 2026 -- still open):
...
I have a simple shell script (~machine-setup/mbp-utm/prepare-vm.sh~) that SSHs into the VM and sets up the basic disk partition, a minimal flake-enabled ~configuration.nix~ seed (root password + SSH), and runs ~nixos-install~. Run it from this directory:
#+begin_src bash
  ./prepare-vm.sh
#+end_src
It prompts for the VM IP if ~NIXOS_IP~ isn't already exported (see "Check IP Address" above). The target disk defaults to ~/dev/vda~ (UTM's virtio-blk); for other hypervisors override with ~NIXOS_BLOCK_DEVICE=sda ./prepare-vm.sh~ (check ~lsblk~ inside the installer).
...
#+begin_src bash
  ssh -o PubkeyAuthentication=no \
      ...
      -o StrictHostKeyChecking=no \
      "root@${NIXOS_IP}" \
      "sudo nixos-rebuild switch --flake \"/nix-config#nixos-utm\""
#+end_src

*** Troubleshooting: narHash mismatch on first switch
If the first switch fails with ~error: mismatch in field 'narHash'~ for the /same/ rev of some input, it's a fetcher-generation gap: for repos carrying Git LFS files, GitHub tarballs (smudged files) and plain git checkouts (pointer files) hash differently, and the installer ISO's Nix can sit on the other side of that behaviour line from the Nix that wrote ~flake.lock~. This bit ~niri~ once (LFS-tracked wiki PNGs, since removed upstream; resolved here by updating the input past the removal -- the tarball and git hashes now agree, verified for both ~niri-unstable~ and ~niri-stable~).

If it ever recurs on some input, the one-time workaround is to let the VM resolve the same rev itself by appending, e.g.:
#+begin_src bash
  --override-input <input-path> 'github:<owner>/<repo>/<rev-from-flake.lock>'
#+end_src
After that switch the system runs the flake-pinned Nix, the locked hashes verify natively, and no override is needed again. Expect the affected package to build from source that once (the overridden tree won't hit binary caches).

*** Troubleshooting: ~executing 'git': No such file or directory~
The first switch vendors cargo git-dependencies (e.g. niri -> Smithay) at eval time, and Nix's git fetcher shells out to the ~git~ binary -- which the minimal seed system lacks. ~prepare-vm.sh~ now bakes ~git~ into the seed config; for a VM installed before that, run the switch with git on PATH once:
#+begin_src bash
  nix shell nixpkgs#git -c nixos-rebuild switch --flake /nix-config#nixos-utm ...
#+end_src
After the first successful switch the full system ships git, so this cannot recur.

** Subsequent Updates
After the first switch, everything runs from inside the VM -- no host involvement needed.

- System: ~nh os switch /nix-config~ (~nh~ finds the ~nixos-utm~ target by hostname), or plain ~nixos-rebuild switch --flake /nix-config#nixos-utm~.
- User level: ~hm switch~ -- the dispatcher (~apps/hm/dispatcher.sh~) resolves the ~nixos-utm~ hostname to the standalone ~homeConfigurations."ryota@nixos-utm"~ profile, which is the same stripped-down ~user-config/ryota/vm.nix~ the system build embeds.

If the private input isn't reachable from this machine (e.g. no personal SSH key), append the stub override described in [[../mbp-parallels/README.org]] -- the same degraded-build flow applies to UTM.
...
#+begin_src bash
  ...
  n (follow prompts) # i.e. Partition number: 1
  ...
  n (follow prompts) # i.e. Partition number: 2
                     #      -> First sector: (keep empty and use default)
...
Also, ensure to run the following to get the swap in place correctly. NOTE: The ~-L swap~ label is required -- ~nixos-config/mbp-vm/hardware.nix~ activates swap via ~/dev/disk/by-label/swap~ (with ~nofail~, so a missing label silently means no swap rather than a boot failure).
#+begin_src bash
  # Format the swap partition (keeping the label the config mounts by),
  # and again enable swap
  ❯ sudo mkswap -L swap /dev/vda2; sudo swapon /dev/vda2
  Setting up swapspace version 1, size = 8 GiB (8588881920 bytes)
  LABEL=swap, UUID=7c95d918-2dbf-4751-9ae3-806588a9a9ab
#+end_src
If swap was ever formatted without the label, it can be fixed in place with ~sudo swaplabel -L swap /dev/vda2~.
...
- Even with spice-vdagent, the display resolution didn't get updated when UTM window was resized -- it seems to have been fixed after restart of the host system? (The guest side -- ~spice-vdagentd~ -- is enabled declaratively via ~nixos-config/modules/virtual-machine/common.nix~.)
- rebuild doesn't always update dconf setup; only when a full rebuild is triggered due to the configuration change
- LIBGL software rendering is required even when graphics acceleration is set up from the host -- this is now handled declaratively (~LIBGL_ALWAYS_SOFTWARE=1~ in ~nixos-config/modules/virtual-machine/utm.nix~), no manual env var needed.
