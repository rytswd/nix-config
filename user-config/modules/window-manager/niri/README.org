...
**** Power key
By default niri takes a ~handle-power-key~ *block* inhibitor from logind so that ~XF86PowerOff~ can be bound like any other key. That is only useful if you actually bind it. I don't, so the button ended up doing nothing at all: logind was inhibited and niri swallowed the key.

The symptom was confusing on a laptop. The power button is an ACPI wake source below the OS, so pressing it while suspended still woke the machine -- but nothing could put it back to sleep, which looked like "the button wakes it up incorrectly" with the lid shut.

~disable-power-key-handling~ makes niri leave the inhibitor alone, so logind handles the key per ~HandlePowerKey~ (set to ~suspend~ in ~nixos-config/modules/machine-specific/laptop.nix~). Doing it in logind rather than as a niri bind means it still works when the session is locked, when the lid is closed, and if niri is wedged.

NOTE: ~HandlePowerKey~ must be set alongside this. logind's default is ~poweroff~, so dropping the inhibitor without setting it would turn the button into a hard power-off.
...
This would mean that xkb layout would have a single entry, and everything else would be triggered by other layers such as xremap or fcitx. I have some issues with propagating the fcitx event down, and thus not using this.
...
This is what I had before. Dvorak and US keys that can be toggled as necessary.
 #+begin_src kdl
        xkb {
            layout "us,us"
            variant "dvorak,"
...
NOTE: This uses a custom keymap. It was less reliable than I liked, and thus it's not what I use.
 #+begin_src kdl :tangle no :noweb-ref x
        xkb {
...
This assumes the ~output.kdl~ file to be available in the XDG directory, so that it can be included as a part of the spec. Device specific display is defined in separate files in this directory.
...
**** Note about external display
External display is also configured per device. This is because the size of the embedded display would change the x alignment.
...
Also, ~open-maximized-to-edges~ applies to some windows by default. I do not really use it anyways, so defaulting it to false.
#+begin_src kdl
  window-rule {
      opacity 0.95
...
For Ghostty, I'm setting the default column to be bigger.
#+begin_src kdl
  window-rule {
...
For Emacs, I'm setting the default column to be even bigger.
#+begin_src kdl
  window-rule {
      match app-id="emacs"
...
:header-args: :tangle config.kdl :noweb yes
:END:
Also known as MRU (Most Recently Used)
...
**** Noctalia
Ref: https://noctalia.dev/
Use Noctalia shell for most of the setup. Noctalia is made available via home-manager, it used to have systemd but is now deprecated. Each compositor is responsible for launching it.
#+begin_src kdl
  spawn-at-startup "noctalia"
#+end_src

NOTE: v5 renamed the binary from ~noctalia-shell~ to ~noctalia~. The old v4 launcher is kept untangled (=:tangle no=) below in case of a rollback to the ~legacy-v4~ branch.
#+begin_src kdl :tangle no
  spawn-at-startup "noctalia-shell"
#+end_src

**** Other
NOTE: This is old setup.
For wallpaper setup using ~awww~, I need the daemon to be running.
#+begin_src kdl :tangle no
...
#+begin_src kdl :noweb-ref old :tangle no
...
:END:
Noctalia based commands. v5 exposes runtime control through ~noctalia msg ...~ (the v4 ~ipc call~ interface is gone). Panels are toggled by id, with an optional trailing context/query: the launcher's ~/emo~ prefix opens the emoji provider, and ~control-center notifications~ jumps straight to the notification-history tab.
#+begin_src kdl
  Alt+Space         { spawn-sh "noctalia msg panel-toggle launcher"; }
  Super+Shift+F12   { spawn-sh "noctalia msg panel-toggle clipboard"; }
  Super+Alt+Backslash { spawn-sh "noctalia msg panel-toggle launcher \"/emo\""; }
  Super+Shift+Backslash { spawn-sh "noctalia msg panel-toggle control-center notifications"; }
#+end_src

NOTE: The old v4 ~ipc call~ bindings are kept untangled below for reference / rollback. The explicit =:noweb-ref v5-archive= keeps them out of the tangled =<<key-bindings-commands>>= expansion (otherwise they would inherit this section's =:noweb-ref key-bindings-commands=).
#+begin_src kdl :tangle no :noweb-ref v5-archive
  Alt+Space         { spawn "noctalia-shell" "ipc" "call" "launcher" "toggle"; }
  Super+Shift+F12   { spawn "noctalia-shell" "ipc" "call" "launcher" "clipboard"; }
  Super+Alt+Backslash { spawn "noctalia-shell" "ipc" "call" "launcher" "emoji"; }
  Super+Shift+Backslash { spawn "noctalia-shell" "ipc" "call" "notifications" "toggleHistory"; }
#+end_src

+I use ~fuzzel~ for my launcher.+
+I moved to use Rofi (with Wayland support) so that I can show more items in one pane.+
+I'm considering to move to use a dedicated solution.+
NOTE: This is the old code before I used Noctalia.
#+begin_src kdl :noweb-ref x
  Alt+Space         { spawn "rofi" "-show" "drun"; }
  Alt+Shift+Space   { spawn "rofi" "-show" "window"; }
#+end_src

There are times some commands stop working. I'm making sure at least I get a terminal I can run stuff from.
#+begin_src kdl
  XF86Launch1 { spawn "ghostty"; }
#+end_src

NOTE: This is the old setup before Noctalia.
...
Change the input using my custom script ~niri-toggle-input-method~. This changes the input method from "Main" to "JP" in Fcitx setting, as well as Dvorak keyboard layout to Qwerty on XKB.
#+begin_src kdl
  F9 { spawn "niri-toggle-input-method"; }
#+end_src
...
*** Visibility
:PROPERTIES:
:header-args: :tangle no :noweb-ref key-bindings-visibility
:END:
#+begin_src kdl
    // NOTE: These key bindings are designed so that they are difficult to use,
    // and should not conflict with other key bindings. In order to use this
    // in real scenario, I would need key remapping solutions like Xremap.
    Ctrl+Shift+Super+0      { toggle-window-rule-opacity; }
#+end_src
...
#+begin_src kdl
    ...
    Shift+XF86KbdLightOnOff { spawn "niri" "msg" "action" "power-off-monitors"; }
