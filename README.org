...
*** For NixOS in a VM (on Mac)
One shared guest configuration (~nixos-config/mbp-vm~) serves two hypervisors: UTM (~#nixos-utm~) and Parallels Desktop (~#nixos-parallels~), differing only in the guest-tools leaf and hostname. For the UTM setup, see [[machine-setup/mbp-utm/README.org]]; for Parallels (including the no-Nix-on-host work-machine flow), see [[machine-setup/mbp-parallels/README.org]].
...
#+html:<details>
#+html:<summary><b>Day-to-day updates</b></summary>
#+html:<br>

After the first bootstrap the experimental features are configured permanently, so the ~NIX_CONFIG~ prefix is no longer needed. The first switch also installs a couple of shell entry points so subsequent updates don't need to be run from inside the repo clone:

| Command | Where it works | What it does |
|---+---+---|
| ~hm-switch~ | everywhere | Shell alias for ~nix run '<repo>#hm' -- switch~. The ~hm~ dispatcher resolves the profile from the current user/hostname, prefers ~nh~ when available, falls back to plain ~home-manager~, and timestamps the backup extension. This is the portable entry point. |
| ~nh home switch~ | personal machines | ~$NH_HOME_FLAKE~ points at the repo and the flake exposes ~homeConfigurations."ryota@<hostname>"~, which is exactly what ~nh~ auto-probes -- so no flake path or ~-c~ is needed. |
| ~nhs~ | coder / devspace | Shell alias for ~nh home switch -c <coder profile> -b … -- --impure~. ~nh~ has no env var for ~-c~ or ~--impure~, and the workspace's ~$USER~ / hostname are not knowable at flake-authoring time, so a bare ~nh home switch~ cannot resolve there -- hence the alias. |

From inside a local clone the dispatcher can also be invoked directly:
...
** 📚 Runbooks
Generic operational flows live in [[docs/runbooks/][docs/runbooks/]] as standalone runbooks, each the single authoritative copy of its flow; machine documents under ~machine-setup/~ and ~nixos-config/~ link to them and keep only their machine-specific deltas.

- [[docs/runbooks/credential-bootstrap.org][Credential bootstrap]] :: Whether a fresh machine builds /full/ (the private input reachable over SSH) or /degraded/ (the public stub substitutes), and how to upgrade later.
- [[docs/runbooks/bare-metal-install.org][Bare-metal install]] :: From a NixOS installer USB to a fully-rebuilt machine, with the guided ~install~ app walking each phase.
- [[docs/runbooks/remote-provision.org][Remote provisioning]] :: Build on one machine, deploy to another over SSH -- first install via ~provision~ (nixos-anywhere), day-2 via ~deploy~.
- [[docs/runbooks/secrets-enrolment.org][Secrets enrolment]] :: Per-instance age recipients for hosts that cannot present a YubiKey -- enrol, use, revoke, with the registry keeping drift visible.
- [[docs/runbooks/binary-cache.org][Binary cache]] :: The self-hosted cache: reading is wired in for every consumer already, pushing is for provisioner-class machines only.
...
| Toolbar           | macOS default | [[https://github.com/noctalia-dev/noctalia][noctalia]]    |
| App Launcher      | [[https://www.raycast.com/][Raycast]]       | [[https://github.com/noctalia-dev/noctalia][noctalia]]    |
